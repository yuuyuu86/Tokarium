...
仕様の元になった案は `Tokarium_仕様書_案.md`。利用者向けの説明は [README.md](README.md)。
...
| 項目 | 内容 |
|---|---|
| 通貨換算 | 入力1・出力1・キャッシュ書込0.25・キャッシュ読込0.1 の重みで合計し、500,000トークン＝1コイン（2026年9月25日にレートを変える前に得た分は10,000トークン＝1コインのまま）。付与上限なし |
...
| 繁殖 | 元気な成魚が同じ種類で2匹以上・水質50以上で、稚魚が1〜3匹生まれることがある（1時間あたり2%・24時間に1回まで）。ペアを決めた魚はその相手とだけ繁殖する |
...
- お店の魚 33 種（季節・隠れた魚・記念の魚を合わせて 54 種）、装飾 57 種。`Game/Catalog.swift` に設計図（体形・ひれ・模様／形のパーツ）で定義し、
...
- 品種改良: 魚は色の遺伝子を2つ持ち、両親から1つずつ受け継ぐ（`Game/Genetics.swift`）。7色×組み合わせで1種類につき13品種。
  お店の魚の2匹に1匹は色の遺伝子をかくし持ち（種類ごとの「出やすい色」になりやすい）、生まれるときにまれに突然変異する。お世話の画面でペアを決めると、その2匹だけで繁殖し、生まれうる品種の確率も見られる
- 飼育員ランク（1〜20, `Game/Keeper.swift`）: 餌やり・水換え（1日の回数に上限）・誕生・成長・図鑑や品種の登録・実績・ミッションで経験値。
  高い魚・装飾・水槽の拡張・設備はランクが上がるとお店に並ぶ。ランクができる前のデータは記録から経験値を見積もる
- 毎日3つ・毎週2つのミッション（`Game/Quests.swift`）。AIの利用量やコインの支払いは数えない。ごほうびは経験値・餌・薬・かけら。かけらは限定の装飾6つと交換
